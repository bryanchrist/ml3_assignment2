...
model_ft.fc = nn.Sequential(
    nn.Linear(num_ftrs, 1300),  # Additional linear layer with 700 output features
    nn.LeakyReLU(negative_slope=0.01, inplace=True),        
    nn.Dropout(0.2),               # Dropout layer with 60% probability
    nn.Linear(1300, num_classes)    # Final prediction fc layer
)
...
optimizer_ft = optim.Adam(model_ft.fc.parameters(), lr=0.0001, weight_decay=0.00001)
exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=10, gamma=0.1)
#model_ft = nn.DataParallel(model_ft)
#model_ft.load_state_dict(torch.load('saved_model_final.pth'))
model_ft.load_state_dict(torch.load('resnext_v5.pth'))
model_ft = nn.DataParallel(model_ft)
...
#             preds = preds.max(dim=0)
        ...
        if min_valid_loss > valid_loss:
            ...
            # Saving State Dict
            torch.save(model.module.state_dict(), 'resnext_v5.pth')
...
generate_submission(predictions, data_dir + '/sample_submission.csv', 'my_submission_resnext_v5.csv')