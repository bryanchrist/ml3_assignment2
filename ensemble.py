...
device1 = torch.device("cuda:0")  # Use GPU 0
...
model_ft.fc = nn.Sequential(
    nn.Linear(num_ftrs, 1600),  # Additional linear layer with 700 output features
    nn.LeakyReLU(negative_slope=0.01, inplace=True),        
    nn.Dropout(0.2),               # Dropout layer with 20% probability
    nn.Linear(1600, num_classes)    # Final prediction fc layer
)
model_ft.load_state_dict(torch.load('resnext1600.pth'))
#model_ft = nn.DataParallel(model_ft)
model_ft = model_ft.to(device1)
model_ft.eval()

model_ft2 = torch.hub.load('pytorch/vision:v0.10.0', 'resnext101_32x8d')
num_classes=120
...
model_ft2.load_state_dict(torch.load('resnext_v4.pth'))
#model_ft2 = nn.DataParallel(model_ft2)
model_ft2 = model_ft2.to(device2)
model_ft2.eval()
...
model = MyEnsemble()
optimizer_ft = optim.Adam(model.parameters(), lr=0.0001, weight_decay=0.005)
...
def train_model(model, optimizer, scheduler, train_loader, valid_loader, loss_module, num_epochs=100):
    ...
    for data_inputs, data_labels in valid_loader:
        data_inputs = data_inputs.to(device1)
        data_inputs2 = data_inputs.to(device2)
        data_labels = np.array(data_labels)
        data_labels = torch.from_numpy(data_labels)
        data_labels = data_labels.to(device2)

        ## Step 2: Run the model on the input data
        preds1 = model_ft(data_inputs)
        preds2 = model_ft2(data_inputs2)
        preds1 = preds1.to(device2)
...
#train_model(model, optmizer_ft, exp_lr_scheduler, train_loader, valid_loader, criterion, num_epochs=25)
model.load_state_dict(torch.load('ensemble.pth'))
model = model.to(device2)
...
# def test_model(model1, model2, data_loader):
...
#             preds1 = model1(data_inputs)
#             preds2 = model2(data_inputs)
...
generate_submission(predictions, data_dir + '/sample_submission.csv', 'my_submission_ensemble_v2.csv')